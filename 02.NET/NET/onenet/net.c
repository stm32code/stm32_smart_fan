...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	//有效数据
	memset(text, 0, sizeof(text));
	sprintf(text, "\"pm2_5\":\"%d\",", Data_init.pm25); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"pwm\":\"%d\",", turn_init.Turn_Num); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"flage\":\"%d\",", Data_init.Flage); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fan\":\"%d\",", device_state_init.fan_state); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"time\":\"%d\",",  device_state_init.tiem); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"time_s\":\"%d\"", device_state_init.time_state); 
	strcat(buf, text);
